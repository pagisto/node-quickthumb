...
qt.convert = function(options, callback) {
	...
	if (options.background) im_options.customArgs.push('-background ' + options.background)
	if (options.extent) im_options.customArgs.push('-gravity center -extent ' + options.width + 'x' + options.height)
        ...
        send_if_exists(dst, function() {
            var dims = dim.split(/x/g),
                opts = {
                    ...
                    width: dims[0],
                    height: dims[1],
					type: (req.query.type !== undefined && ['resize', 'crop'].indexOf(req.query.type) > 0) ? req.query.type : options.type,
					quality: (req.query.quality !== undefined && !isNaN(parseFloat(req.query.quality))) ? parseFloat(req.query.quality) : options.quality,
